...
## ✅ Acceptance Criteria
1. [ ] เมื่อ WebRTC ต่อสำเร็จและห้องเปลี่ยนเป็น `active` connection ต้อง**คงอยู่** (ตรวจด้วย indicator `connType === "webrtc"` ค้างตลอดเกม)
2. [ ] `cleanupWebRTC()` ถูกเรียกเฉพาะ: component unmount, เกมจบ (`finished`/`expired`), หรือ connection fail จริง
3. [ ] กรณี WebRTC ล้มเหลว (STUN ไม่ผ่าน/NAT ปิด) ยัง fallback เป็น polling ได้เหมือนเดิม
4. [ ] การเดินหมากระหว่างสองเครื่องผ่าน data channel แสดงผลฝั่งตรงข้ามภายใน < 1 วินาที (ทดสอบจริง 2 browser/2 เครื่องบน local)
5. [ ] ไม่มี memory/interval leak: ออกจากหน้าห้องแล้ว interval และ peer connection ถูกปิดครบ

## 🛠 Technical Tasks (งานพัฒนาที่ต้องทำ)
- [ ] แยก WebRTC lifecycle ออกจาก `useEffect([status])` — เช่น trigger setup ครั้งเดียวเมื่อเข้าสถานะ `connecting` (guard ด้วย ref) และผูก cleanup กับ unmount-only effect (`useEffect(() => cleanup, [])`)
- [ ] เพิ่มการเรียก `cleanupWebRTC()` อย่างชัดเจนเมื่อ status เป็น `finished`/`expired`
- [ ] ตรวจ `webrtcActive` / `connType` ให้สอดคล้องกับ lifecycle ใหม่ (ไม่ค้างค่า stale)
- [ ] ทดสอบ E2E บน local: host + guest คนละ browser เล่นจบเกมโดย `connType` เป็น `webrtc` ตลอด, ทดสอบ fallback โดย block STUN
- [ ] บันทึกผลการทดสอบใน PR
