...
## ✅ Acceptance Criteria
1. [ ] Client ส่ง ICE candidate **ทีละตัว** (payload เล็ก ไม่มี read-modify-write ฝั่ง client)
2. [ ] Server (`signal/route.ts`) เป็นผู้ append candidate ลง `ice_candidates` แบบ atomic (เช่น `SET ice_candidates = ice_candidates || :new` ใน SQL เดียว) — ไม่มี candidate หายแม้ POST ซ้อนกัน
3. [ ] GET `/signal` ยังคงคืน candidates ของฝั่งตรงข้ามครบทุกตัวตามลำดับ
4. [ ] จำนวน candidates ต่อ record มีเพดาน (เช่น ≤ 30 ตัว) เพื่อกัน payload บวม — เกินแล้วปฏิเสธ
5. [ ] ทดสอบจริง: host/guest ต่อกันสำเร็จติดต่อกัน ≥ 5 ครั้งบนเครือข่ายเดียวกัน และ log ยืนยันว่า candidates ทุกตัวถูกเก็บครบ

## 🛠 Technical Tasks (งานพัฒนาที่ต้องทำ)
- [ ] เพิ่มรูปแบบ request ใหม่ใน `POST /api/battle/rooms/[code]/signal`: field `iceCandidate` (ตัวเดียว) — server append ด้วย jsonb concat ใน query เดียว
- [ ] ตัด logic GET-ก่อน-POST ใน `uploadIceCandidate` ฝั่ง client ให้เหลือ POST candidate เดียวตรงๆ
- [ ] คง compatibility ของ field `sdpOffer`/`sdpAnswer` เดิม
- [ ] เพิ่มเพดานจำนวน candidates + ตรวจรูปร่าง candidate ขั้นต่ำ (มี `candidate`/`sdpMid` เป็น string)
- [ ] ทดสอบการเชื่อมต่อซ้ำหลายรอบ + กรณี candidates มาถี่พร้อมกัน
