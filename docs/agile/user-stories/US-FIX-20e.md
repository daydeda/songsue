...
## ✅ Acceptance Criteria
1. [ ] การ update หมากใช้ optimistic concurrency: `UPDATE ... WHERE id = :id AND status = 'active' AND current_turn = :expected` และเช็ค affected rows — ถ้า 0 แถวให้ตอบ 409/400 ไม่ใช่เขียนทับ
2. [ ] `finalizeGameInDb` เป็น idempotent: เงื่อนไข `WHERE status = 'active'` (หรือเทียบเท่า) — เรียกซ้ำกี่ครั้งสถิติก็ถูก update เพียงครั้งเดียว
3. [ ] `updatePlayerStats` เปลี่ยนเป็น upsert (`INSERT ... ON CONFLICT (user_id, game_type) DO UPDATE`) พร้อม atomic increment (`wins = game_stats.wins + 1` ฯลฯ) ใน statement เดียว
4. [ ] ยิง `POST /move` ซ้อนกัน 2 request (turn เดียวกัน) → สำเร็จเพียง 1, อีกอันได้ error ชัดเจน; board ใน DB สอดคล้องกับ move ที่สำเร็จ
5. [ ] จำลอง forfeit ด้วย poll `GET /state` พร้อมกันสองฝั่ง → สถิติ win/loss เพิ่มขึ้นฝั่งละ 1 เท่านั้น (ไม่นับซ้ำ)
6. [ ] มี unit test ครอบ `updatePlayerStats` (upsert/increment) และ integration test สำหรับ move ที่ชนกัน (รันบน local DB)

## 🛠 Technical Tasks (งานพัฒนาที่ต้องทำ)
- [ ] แก้ `POST /move`: รวมเช็ค turn + apply move เป็น conditional UPDATE เดียว, อ่าน affected rows ผ่าน `.returning()`
- [ ] แก้ `finalizeGameInDb` ใน `src/lib/games/stats-helper.ts`: update room แบบมีเงื่อนไข status และ **ข้าม** การอัปเดต stats เมื่อไม่มีแถวถูกเปลี่ยน
- [ ] เขียน `updatePlayerStats` ใหม่เป็น single upsert + atomic increments (คำนวณ `win_streak`/`best_streak` ใน SQL expression)
- [ ] ตรวจจุดเรียก lazy-forfeit ใน `GET /state` และ `POST /move` ให้ใช้ path ที่ idempotent เดียวกัน
- [ ] เพิ่ม tests ตาม AC ข้อ 6 (vitest + local DB)
