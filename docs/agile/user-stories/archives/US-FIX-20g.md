...
## ✅ Acceptance Criteria
1. [x] เมื่อ `connType === "webrtc"` state poll ผ่อนลงเหลืออย่างมาก 1 ครั้ง/30 วิ (ใช้เป็น reconciliation กับ server เท่านั้น) หรือหยุดจนกว่า channel จะหลุด
2. [x] Signaling poll (1 วิ) **หยุดทันที**เมื่อ data channel เปิดสำเร็จ และกลับมาเฉพาะตอน renegotiate
3. [x] โหมด fallback (ไม่มี WebRTC) poll ไม่ถี่กว่า 5 วินาที และหยุดสนิทเมื่อห้อง `finished`/`expired` หรือ tab ไม่ active (`document.visibilityState`)
4. [x] Endpoint ที่ถูก poll ยังเป็น O(1) ต่อ query (indexed lookup — เป็นอยู่แล้ว ห้าม regress)
5. [x] วัดผลก่อน/หลังบน local: จำนวน request ต่อนาทีต่อห้องขณะเล่นผ่าน WebRTC ลดลง ≥ 80% และเกมยังเล่นได้ลื่นทั้งสองโหมด

## 🛠 Technical Tasks (งานพัฒนาที่ต้องทำ)
- [x] เพิ่ม logic ปรับ interval ตาม `connType` ใน `RoomClient.tsx` (webrtc → 30 วิ/หยุด, polling → 5 วิ)
- [x] เคลียร์ `signalPollInterval` เมื่อ `dc.onopen` และตั้งใหม่เฉพาะเมื่อต้อง renegotiate
- [x] เพิ่ม visibility handling: หยุด poll เมื่อ tab hidden, resume เมื่อกลับมา
- [x] ส่งข้อมูล turn deadline ผ่าน data channel ด้วย เพื่อลดการพึ่ง state poll ระหว่างเกม
- [x] วัดและบันทึกตัวเลข request/นาที ก่อน-หลังลง PR
