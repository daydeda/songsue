...
* `registrationMode` (text, default `'once'`): โหมดการลงทะเบียน (`'once'` = ลงทะเบียนครั้งเดียวเข้าได้ทุกเซสชัน, `'per_session'` = แยกลงทะเบียนแต่ละเซสชัน/แต่ละวัน)
...
* `managedByRoles` (jsonb): สิทธิ์บทบาทผู้จัดการหลักของกิจกรรม (เช่น `['club_president', 'major_president']`)
...
#### 📋 ตาราง `event_sessions`
บันทึกรายละเอียดรอบ/เซสชันของกิจกรรม (เช่น วันที่ 1, วันที่ 2 ของกิจกรรม CAMT LINK หรือ รอบเช้า/รอบบ่าย)
* `id` (uuid, defaultRandom, primaryKey): รหัสรอบกิจกรรม
* `eventId` (uuid, references `events.id` ON DELETE CASCADE, notNull): รหัสกิจกรรมหลัก
* `title` (text): ชื่อรอบกิจกรรม (เช่น "Day 1"; หากเป็น NULL จะอิงตาม "Day N" ในระบบแสดงผล)
* `startTime` / `endTime` (timestamptz, notNull): เวลาเริ่มและจบของรอบนั้น ๆ
* `sortOrder` (integer, default 0, notNull): ลำดับของรอบกิจกรรมในการจัดเรียง
* `quotaWalkIn` (integer): โควต้าจำกัดสำหรับ Walk-in เฉพาะรอบนี้
* `createdAt` / `updatedAt` (timestamptz)
* *ดัชนี (Indexes):* `idx_event_sessions_event` บนฟิลด์ `eventId`, `idx_event_sessions_event_order` บนฟิลด์ `(eventId, sortOrder)`

#### 📋 ตาราง `attendance`
บันทึกการลงทะเบียนและประวัติการสแกนเช็คอินกิจกรรมของนักศึกษา
* `id` (uuid, primaryKey)
* `eventId` (uuid, references `events.id` ON DELETE CASCADE, notNull): รหัสกิจกรรมหลัก (คัดลอกมาเพื่อช่วยความเร็วในการวิเคราะห์สรุป)
* `sessionId` (uuid, references `event_sessions.id` ON DELETE CASCADE, notNull): รหัสรอบกิจกรรมที่เข้าเช็คอิน (เช็คอินแยกรายเซสชัน)
...
* *ดัชนี (Indexes):* `idx_attendance_session_student` (uniqueIndex ป้องกันการเช็คอินซ้ำในรอบเดียวกัน), `idx_attendance_event_student` (index สำหรับการดึงสิทธิ์และรายงานระดับกิจกรรมหลัก), `idx_attendance_student` บนฟิลด์ `studentId`, `idx_attendance_checkin_time` บนฟิลด์ `checkInTime`
...
* `formId` (uuid, references `forms.id` ON DELETE SET NULL): รหัสแบบประเมินที่เป็นที่มาของคะแนนโบนัส/คะแนนการทำแบบประเมิน (สำหรับใช้ดึงคะแนนคืนแบบ Transactional เมื่อแอดมินเปิดแบบประเมินใหม่อีกรอบ)
...
* *ดัชนี (Indexes):* `idx_score_history_event` บนฟิลด์ `eventId`, `idx_score_history_form` บนฟิลด์ `formId` (ใช้ในการลบและคืนคะแนนเฉพาะฟอร์มอย่างรวดเร็ว), `idx_score_history_timestamp` บนฟิลด์ `timestamp` (ปรับปรุงประสิทธิภาพการทำงาน `ORDER BY timestamp DESC` ในหน้ากิจกรรมล่าสุด)
...
### 2.7 กลุ่มตารางระบบปฏิทินและเชื่อมต่อ (Calendar & Feed Tokens)

#### 📋 ตาราง `calendar_entries`
เก็บข้อมูลกิจกรรมเสริม/ข้อมูลบนปฏิทินที่ไม่นับเป็นกิจกรรมเช็คอินหลัก (เช่น สัปดาห์สอบ, กำหนดส่งงาน, ช่วงปิดรับลงทะเบียน)
* `id` (uuid, defaultRandom, primaryKey): รหัสรายการปฏิทิน
* `title` (text, notNull): ชื่อรายการ
* `description` (text): รายละเอียด
* `location` (text): สถานที่จัดกิจกรรม
* `startTime` / `endTime` (timestamptz, notNull): เวลาเริ่มต้นและสิ้นสุด
* `allDay` (boolean, default false): ระบุว่าเป็นกิจกรรมแบบทั้งวัน (All-day) หรือไม่
* `eventId` (uuid, references `events.id` ON DELETE SET NULL): ไอดีกิจกรรมหลักที่เกี่ยวข้อง (ถ้ามี)
* `allowedRoles` (jsonb): สิทธิ์บทบาทผู้ใช้ที่มองเห็น (เช่น `['student']`)
* `allowedMajors` (jsonb): สาขาวิชาที่มองเห็น
* `targetThai` / `targetInternational` (boolean, default true): การแสดงผลแบ่งตามกลุ่มหลักสูตรไทยและต่างประเทศ
* `createdBy` (text): ไอดีผู้ใช้ผู้สร้างรายการ
* `createdAt` / `updatedAt` (timestamptz)
* *ดัชนี (Indexes):* `idx_calendar_entries_start` บนฟิลด์ `startTime`, `idx_calendar_entries_event` บนฟิลด์ `eventId`

#### 📋 ตาราง `calendar_feed_tokens`
เก็บโทเค็นส่วนบุคคลสำหรับการเชื่อมโยงกับปฏิทินภายนอก (Google Calendar, Apple Calendar, Outlook) ผ่าน .ics feed
* `userId` (text, references `users.id` ON DELETE CASCADE, primaryKey): รหัสนักศึกษา/ผู้ใช้
* `token` (text, unique, notNull): โทเค็นสิทธิ์ความปลอดภัยส่วนบุคคลที่ระบบสุ่มสร้างขึ้น (สิทธิ์การเข้าถึงภายนอกแบบอ่านอย่างเดียว)
* `createdAt` (timestamptz, defaultNow)
* `lastUsedAt` (timestamptz): บันทึกเวลาที่มีการส่งคำขอดึงไฟล์ปฏิทินล่าสุด
