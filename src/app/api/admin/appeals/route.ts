...
// first. Gated by VIEW_APPEALS_ROLES (src/lib/strikes.ts): super_admin/admin/smo
// see every appeal; club_president/major_president are scoped to appeals whose
// event they own (EventScopeService, mirroring api/admin/events' list scoping) —
// an appeal with no linked event, or one linked to an event they don't own, is
// hidden from them. Whether a viewer may also resolve (approve/reject) an appeal
// is enforced separately and more narrowly by PATCH /api/admin/appeals/[id]
// (RESOLVE_APPEALS_ROLES excludes smo). Each appeal is tied to one specific
...
export async function GET(req: Request) {
  try {
    ...
    const isUnscoped = myRoles.some((r) => ["super_admin", "admin", "smo"].includes(r));
